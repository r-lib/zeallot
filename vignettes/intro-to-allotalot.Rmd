...
The right-hand side of the assignment expression is a list, vector, or other object. It is important to note that the right-hand side value is recursively coerced to a list structure. Singleton values are not coerced to lists of length 1. As we will see, different syntax is used to unpack vectors and lists, but the difference is minor. 
...
# `rest` is rather long, but feel free to run the example
# and look at the remaining contents of `rest`
class(rest)
length(rest)
```
...
As mentioned above, the right-hand side value is recursively coerced to a list structure. So, in the above example `resids` is a named list not a named vector as `summary.lm` returns. Be mindful of this coercion when unpacking values. 
