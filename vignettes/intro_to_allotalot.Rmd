...
The *allotalot* package defines an operator for *unpacking assignment*,
sometimes called *parallel assignment* or *destructuring assignment*
...
did occur. The second item is the error (if an error occurred) or `NULL` (if
not). Whether or not the original function would have thrown an error, the safe
...
A data frame is simply a list of columns, so the *allotalot* assignment does
...
```{R}
an1 <- data.frame(x = anscombe$x1, y = anscombe$y1)
an2 <- data.frame(x = anscombe$x2, y = anscombe$y2)
an3 <- data.frame(x = anscombe$x3, y = anscombe$y3)
an4 <- data.frame(x = anscombe$x4, y = anscombe$y4)

{ hd1 : hd2 : hd3 : hd4 } %<-% lapply(list(an1, an2, an3, an4), head, n = 3)
...
The `%<-% operator assigned three data frames to three variables, leaving the
data frames intact.

## Unpacking nested values In addition to unpacking flat lists, you can unpack
lists of lists.
...
### Character vectors You can assign individual characters of a string to
variables. Since character strings are vectors, do not forget to exclude braces
...
### Class objects
*allotalot* includes implementations of `destructure` for character strings,
...
Only one collector variable may be specified at each depth of names. If multiple
collectors are specified `%<-%` will raise an error.
